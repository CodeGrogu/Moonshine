...
public sealed class NvencHardwareEncoderPipeline : IVideoEncoderPipeline
{
    ...
    private NvencPreset _preset;
    private NvencTuning _tuning;
    private bool _intraRefreshEnabled;
    private uint _intraRefreshPeriod;
    private uint _intraRefreshCount;
    ...
    public EncoderVendor Vendor => EncoderVendor.NvidiaNvenc;
    ...
    public NvencHardwareEncoderPipeline(
        ...
        NvencPreset preset = NvencPreset.P1_UltraFast,
        NvencTuning tuning = NvencTuning.UltraLowLatency,
        IntPtr d3dDevice = 0
    )
    {
        _width = width;
        ...
        _handle = MoonshineNativeMethods.EncoderCreate((uint)EncoderVendor.NvidiaNvenc, d3dDevice, in config);
    ...
    public bool ConfigureIntraRefresh(bool enable, uint period, uint count)
    {
        lock (_lock)
        {
            ...
            if (res > 0)
            {
                ...
                _intraRefreshPeriod = period;
                _intraRefreshCount = count;
                return true;
            }
            return false;
        }
    ...
    public bool TryRecoverDevice(IntPtr newD3dDevice)
    {
        lock (_lock)
        {
            ...
            _handle = MoonshineNativeMethods.EncoderCreate((uint)EncoderVendor.NvidiaNvenc, newD3dDevice, in config);
            if (_handle != IntPtr.Zero)
            {
                _runtimeState = EncoderRuntimeState.Ready;
                _ = MoonshineNativeMethods.NvencSetTuning(_handle, (uint)_preset, (uint)_tuning);
    ...
    {
        int res = MoonshineNativeMethods.NvencQueryCodecSupport((uint)codec, out uint supported);
