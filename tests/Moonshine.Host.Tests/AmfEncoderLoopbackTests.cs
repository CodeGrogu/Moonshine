...
public class AmfEncoderLoopbackTests
{
    ...
    [SkippableFact]
    public unsafe void Amf_ResolutionChange_DynamicallyReconfiguresDimensions()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            ...
            using var pipeline = new AmfHardwareEncoderPipeline(1280, 720, codec: VideoCodec.HevcMain10, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF 720p pipeline initialisation failed (DRIVER ERROR).");
    ...
    public void Amf_BitrateChange_DynamicallyAdaptsBitrate()
    {
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, bitrateKbps: 5000, d3dDevice: dev);
    ...
    [SkippableFact]
    public void Amf_Flush_ResetsInternalBuffersAndForcesImmediateKeyframe()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
