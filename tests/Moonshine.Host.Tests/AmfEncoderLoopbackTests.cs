...
/// <summary>
/// Dedicated AMD AMF hardware video encoder loopback and lifecycle conformance test suite.
/// Strictly enforces the Hardware Encoder Operational Invariant: live Direct3D 11 GPU submission,
/// AMF state machine handling, structural NALU verification, video decoder loopback, and deterministic pixel validation.
/// </summary>
public class AmfEncoderLoopbackTests
{
    [SkippableFact]
    public void Amf_Initialise_ValidatesStateAndCapabilities()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) or Direct3D 11 runtime unavailable (NOT PRESENT).");

        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, fps: 60, bitrateKbps: 20000, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF encoder session initialization failed on AMD GPU (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_EncodeH264_ProducesStructurallyValidBitstream()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, codec: VideoCodec.H264, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF H.264 encoding unavailable (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_EncodeHEVC_ProducesStructurallyValidBitstream()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, codec: VideoCodec.HevcMain10, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF HEVC encoding unavailable (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_EncodeAV1_GatedByHardwareCapabilities()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");

        int capsRes = MoonshineNativeMethods.EncoderQueryCaps((uint)EncoderVendor.AmdAmf, dev, out var caps);
        Skip.If(capsRes != 0 || (caps.SupportedCodecsMask & 0x08) == 0, "AMF AV1 encoding is not supported on this AMD GPU architecture (UNSUPPORTED CODEC).");
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, codec: VideoCodec.Av1, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF AV1 pipeline initialisation failed (DRIVER ERROR).");
    ...
    [SkippableFact]
    public unsafe void Amf_DecodeLoopback_DecodesAndVerifiesDimensionsAndPattern()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, codec: VideoCodec.HevcMain10, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF HEVC pipeline initialisation failed (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_MultipleFrames_EncodesContinuous30FrameSequence()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, fps: 60, bitrateKbps: 30000, codec: VideoCodec.HevcMain10, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF HEVC pipeline initialisation failed (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_ResolutionChange_DynamicallyReconfiguresDimensions()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            ...
            // 720p pipeline
            using (var pipeline720 = new AmfHardwareEncoderPipeline(1280, 720, d3dDevice: dev))
            {
                Skip.IfNot(pipeline720.IsActive, "AMF 720p pipeline initialisation failed (DRIVER ERROR).");
            ...
            // 1080p pipeline
            using (var pipeline1080 = new AmfHardwareEncoderPipeline(1920, 1080, d3dDevice: dev))
            {
                Skip.IfNot(pipeline1080.IsActive, "AMF 1080p pipeline initialisation failed (DRIVER ERROR).");
            ...
            // 1440p pipeline
            using (var pipeline1440 = new AmfHardwareEncoderPipeline(2560, 1440, d3dDevice: dev))
            {
                Skip.IfNot(pipeline1440.IsActive, "AMF 1440p pipeline initialisation failed (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_BitrateChange_DynamicallyAdaptsBitrate()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
        ...
        try
        {
            using var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, bitrateKbps: 20000, d3dDevice: dev);
            Skip.IfNot(pipeline.IsActive, "AMF pipeline initialisation failed (DRIVER ERROR).");
    ...
    [SkippableFact]
    public void Amf_Drain_FlushesInFlightFramesCleanly()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
    ...
    [SkippableFact]
    public void Amf_Reset_ReinitialisesEncoderState()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        Skip.If(dev == IntPtr.Zero, "AMD GPU (0x1002) unavailable (NOT PRESENT).");
    ...
    [Fact]
    public void Amf_Shutdown_ReleasesResourcesDeterministically()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x1002);
        ...
        try
        {
            var pipeline = new AmfHardwareEncoderPipeline(1920, 1080, d3dDevice: dev);
            pipeline.Dispose();
