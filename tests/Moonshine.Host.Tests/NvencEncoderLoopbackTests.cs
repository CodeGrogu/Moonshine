...
public class NvencEncoderLoopbackTests
{
    ...
    [SkippableFact]
    public void Nvenc_ResolutionChange_DynamicallyReconfiguresDimensions()
    {
        IntPtr dev = MoonshineNativeMethods.D3D11CreateDevice(0x10DE);
        Skip.If(dev == IntPtr.Zero, "NVIDIA GPU (0x10DE) unavailable (NOT PRESENT).");
        ...
        try
        {
            ...
            // 720p pipeline
            using (var pipeline720 = new NvencHardwareEncoderPipeline(1280, 720, d3dDevice: dev))
            {
                Skip.IfNot(pipeline720.IsActive, "NVENC 720p pipeline initialisation failed (DRIVER ERROR).");
            ...
            // 1080p pipeline
            using (var pipeline1080 = new NvencHardwareEncoderPipeline(1920, 1080, d3dDevice: dev))
            {
                Skip.IfNot(pipeline1080.IsActive, "NVENC 1080p pipeline initialisation failed (DRIVER ERROR).");
            ...
            // 1440p pipeline
            using (var pipeline1440 = new NvencHardwareEncoderPipeline(2560, 1440, d3dDevice: dev))
            {
                Skip.IfNot(pipeline1440.IsActive, "NVENC 1440p pipeline initialisation failed (DRIVER ERROR).");
    ...
    public void Nvenc_BitrateChange_DynamicallyAdaptsBitrate()
    {
        ...
        try
        {
            using var pipeline = new NvencHardwareEncoderPipeline(1920, 1080, bitrateKbps: 20000, d3dDevice: dev);
