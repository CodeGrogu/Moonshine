...
void test_native_injector_lifecycle() {
    ...
    assert(bounds.cx_virtual_screen > 0);
    assert(bounds.cy_virtual_screen > 0);
...
void test_native_mouse_move_and_abs() {
    ...
    assert(injector.inject_mouse_move(0, 0));
    assert(injector.inject_mouse_move(5, -5));

    // Invalid dimensions fail-closed
    assert(!injector.inject_mouse_abs(100, 100, 0, 1080));
    assert(!injector.inject_mouse_abs(100, 100, 1920, -5));

    // Valid absolute move (center of 1080p stream)
    assert(injector.inject_mouse_abs(960, 540, 1920, 1080));

    // Multi-monitor offset absolute move (secondary monitor offset)
    assert(injector.inject_mouse_abs(960, 540, 1920, 1080, 1920, 0, 1920, 1080));
}
...
void test_native_mouse_buttons_and_scroll() {
    ...
    // Invalid button
    assert(!injector.inject_mouse_button(0, true));
    assert(!injector.inject_mouse_button(6, true));

    // Valid buttons: Left(1), Right(2), Middle(3), X1(4), X2(5)
    for (uint8_t b = 1; b <= 5; ++b) {
        assert(injector.inject_mouse_button(b, true));
        assert(injector.inject_mouse_button(b, false));
    }

    // Scroll: vertical and horizontal
    assert(injector.inject_mouse_scroll(0, false));
    assert(injector.inject_mouse_scroll(120, false));
    assert(injector.inject_mouse_scroll(-120, true));
}
...
void test_native_keyboard_and_extended_keys() {
    ...
    // Invalid vkey
    assert(!injector.inject_keyboard_key(-1, 0, true, 0));
    assert(!injector.inject_keyboard_key(256, 0, true, 0));

    // Standard key: VK_A (0x41)
    assert(injector.inject_keyboard_key(0x41, 0, true, 0));
    assert(injector.inject_keyboard_key(0x41, 0, false, 0));

    // Extended key: VK_RIGHT (0x27)
    assert(injector.inject_keyboard_key(0x27, 0, true, 0));
    assert(injector.inject_keyboard_key(0x27, 0, false, 0));
}
...
void test_native_stuck_key_release() {
    ...
    assert(injector.inject_mouse_button(1, true));
    assert(injector.inject_keyboard_key(0x11, 0, true, 0)); // VK_CONTROL
    assert(injector.inject_keyboard_key(0x57, 0, true, 0)); // 'W' key
...
void test_c_abi_input_exports() {
    ...
    assert(moonshine_input_get_virtual_desktop_bounds(injector, &bounds) == 1);
    assert(bounds.cx_virtual_screen > 0);

    assert(moonshine_input_inject_mouse_move(injector, 0, 0) == 1);
    assert(moonshine_input_inject_mouse_abs(injector, 100, 100, 1920, 1080, 0, 0, 1920, 1080) == 1);
    assert(moonshine_input_inject_mouse_button(injector, 1, 0) == 1);
    assert(moonshine_input_inject_mouse_scroll(injector, 120, 0) == 1);
    assert(moonshine_input_inject_keyboard(injector, 0x41, 0, 0, 0) == 1);
