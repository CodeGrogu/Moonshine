# Moonshine Implementation Backlog

> This file contains the long-form implementation and verification backlog moved out of `TODO.md`.
>
> `TODO.md` is now the active execution queue. Completed historical TODO checkpoints are not treated as proof of current end-to-end readiness unless their acceptance evidence remains valid.
>
> **Important correction:** the previous `TODO-023` entry treated Issue #81 as completed. The 26 August 2026 audit found that Issue #81 is not complete. The active truth for Issue #81 is maintained in `TODO.md`, and the backlog item below remains incomplete until its acceptance evidence is actually produced.
...
- Closed GitHub issues must agree with the active task state. If acceptance criteria are incomplete, the issue is not Done.
