...
> [!NOTE]
> **PROJECT STATUS**
> Moonshine is in active development (v0.5.6-alpha). The application is fail-closed. Input transmission is not yet wired to MNBP v1 transport.

Moonshine incorporates a dedicated, ultra-low-latency 1000Hz (1.0ms) input polling and dispatch pipeline delivering immediate mouse, keyboard, and gamepad state synchronisation to the remote Moonshine host.
...
InputPollingEngine (Dedicated Real-Time Highest-Priority Polling Thread)
                     ...
                     │
                     ▼
Binary Input Packets (Packed StructLayout byte serialisation)
                     │
...
All input structures are packed with 1-byte alignment (`Pack = 1`) and serialised using big-endian binary primitives for direct network transmission:
...
3. **Zero-Allocation Stack Buffers**:
   - Packets are serialised directly into stack-allocated spans (`stackalloc byte[32]`) and dispatched directly to the UDP socket pipeline.
